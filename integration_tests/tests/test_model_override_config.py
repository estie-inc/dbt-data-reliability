...
def test_override_test_sample_all(
    ...
    test_result = dbt_project.test(
        ...
        model_config={"meta": {"test_sample_row_count": None}},
    )
...
@pytest.mark.skip_targets(["clickhouse"])
def test_override_test_sample_limited(
    test_id: str, dbt_project: DbtProject
):
    null_count = 20
    data = [{COLUMN_NAME: None} for _ in range(null_count)]
    ...
    test_result = dbt_project.test(
        ...
        model_config={"meta": {"test_sample_row_count": 3}},
    )
    assert test_result["status"] == "fail"
